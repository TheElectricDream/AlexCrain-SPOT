...
y_median = -0.01050488
yaw_median = 0
path_to_weights_const = './final_models/FinalTrainedLargeSPOTNet.h5'
...
#            print("Zed Camera Capture Time:" + str(zedStop - zedTimeStart))
            ...
            if True: #timestamp_diff > 10.0 and not positional_data == '':
  ...
  #              start = time.time()
#                output = pose_net.predict(stacked_stereo)
...
#                yaw = ((float(output[2]) + yaw_median) * 2 * math.pi) - math.pi
                seen = float(output[3])
                print("X: " + str(x) + " Y: " + str(y) + " Yaw: " + " In View: " + str(seen))
